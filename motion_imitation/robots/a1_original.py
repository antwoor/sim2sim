...
if __name__ == '__main__':
    ...
    robot.ResetPose(add_constraint=1)
    time.sleep(1)
    ...
    for i in range(11):
        while True:
            print(f"\nTesting joint {i}")
            print(f"Motor name: {robot._GetMotorNames()[i]}")
            
            # Create a test action vector (all zeros except one joint)
            test_action = -np.ones(12)*0.1
            for j in range(10):
                test_action[i] = np.random.randint(-3, 3)
                print(test_action[i], "test_action",i, "joint name", robot._GetMotorNames()[i])
                robot.Step(test_action)
                time.sleep(0.1)  # Set only one joint to move
